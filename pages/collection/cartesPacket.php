...
<head>
    ...
    <link rel="stylesheet" href="blocs/series/cartes/cartes.css"/>
    <title>Series</title>
</head>
<body>
<div class="container">
        <div class="row">
            <?php
                // if (isset($_GET['id_serie' and 'id_bloc'])) {
                    ...
                    $req = $db->prepare("SELECT * FROM carte ORDER BY id_carte ASC");
                    $req->execute();
                ...
                <img id="btnPopup<?php echo $result["id_carte"] ?>" class="btnPopup" src="<?php echo $result["image_carte"] ?>" width="100%" height="100%">
                    <div id="overlay<?php echo $result["id_carte"] ?>" class="overlay"><span id="btnClose<?php echo $result["id_carte"] ?>" class="btnClose" width=50px>&times;</span>
                        <div id="popup<?php echo $result["id_carte"] ?>" class="popup">
                            ...
                            <div class="infoPopup">
                                <form name="ajoutCartePacket" action="ajoutCartePacket_traitement.php" method="POST">
                                    <!-- <h2><p>Nom de la carte : <span name="leNom"><?php echo $result["nom_carte"] ?></span></p></h2>
                                    <h2><p>id de la carte : <span name="id_carte"><?php echo $result["id_carte"] ?></span></p></h2>
                                    <h2><p>id packet : <span name="id_packet"><?php echo $id_packet ?></span></p></h2> -->
                                    <input type="text" name="leNom" value="<?php echo $result["nom_carte"] ?>"></input>
                                    <input type="int" name="id_carte" value="<?php echo $result["id_carte"] ?>"></input>
                                    <input type="int" name="id_packet" value="<?php echo $id_packet ?>"></input>
                                    <input type="text" name="image_carte" value="<?php echo $result["image_carte"] ?>"></input>
                                    ...
                                    <input id="ajoutCarte" type="submit" name="ajouterCarte" value="Ajouter au packet <?php echo $id_packet ?>" onclick="ajoutPacket()"/>                            
                                </form>
                            </div>
                        </div> 
    ...
    <script src="blocs/series/cartes/script.js"></script>
</body>
</html>